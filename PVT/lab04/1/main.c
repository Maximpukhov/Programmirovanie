...
void calculate_forces_paralel(struct particle *p, struct particle *f, float *m, int n,int n_threads)
{
...
double main_run_paralel(int n, int n_threads)
{
    ...
    for (double t = 0; t <= 1; t += dt)
    { // Цикл по времени (модельному)
        calculate_forces_paralel(p, f, m, n,n_threads); // Вычисление сил – O(N^2)
        move_particles(p, f, v, m, n, dt); // Перемещение тел O(N)
    }
    ttotal += wtime();
    printf("# NBody (n=%d)\n", n);
