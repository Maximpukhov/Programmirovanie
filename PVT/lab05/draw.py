...
def draw(filenames, labels, colors):
    ...
    i = 0
    for (fname, datalabel, color) in zip(filenames, labels, colors):
        data = np.loadtxt(fname)
        x = data[:, 0]
        y = data[:, 1]

        #ax.plot(x,y,'-o',markersize=1,c="green")
        ax.plot(x,y,'-o',markersize=1,linewidth=0.5,label=datalabel, color=color)
        #x.bar(x, y);
...
if __name__ == "__main__":
    draw(["1000_10_8.txt", "10000_10_9.txt"], ["1000","10000"], ["gray", "yellow"])